(() => {
  ...
  function attachUI() {
    ...
    // queue position updater (optional)
    setInterval(async () => {
      try {
        const r = await fetch('/api/store/jobs'); const j = await r.json();
        (j.items||[]).forEach(job => {
          const item = document.querySelector(`.dl-item[data-job-id="${job.id}"] [data-role="msg"]`);
          if (!item) return;
          if (job.status === 'queued' && job.queue_position != null) {
            item.textContent = `Queued (#${job.queue_position})`;
          }
          if (job.status === 'canceled' || job.status === 'completed' || job.status === 'error') {
            removeDockItem(job.id);
          }
        });
      } catch {}

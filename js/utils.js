...
safeRenderer.link = (href, title, text) => {
  try {
    ...
    return `<a href="${escapeHTML(url)}"${ti} target="_blank" rel="noopener noreferrer">${t}</a>`;
  } catch { return `<a>${escapeHTML(text||'')}</a>`; }
};
...
function showToast(text){
  try{
    if(typeof text==='string' && text.startsWith('Added to memory:')){
      const m = text.match(/^Added to memory:\s*"([\s\S]*)"$/);
      const mem = m ? m[1] : text.replace(/^Added to memory:\s*/,'');
      if(typeof window.addMemoryNotice === 'function'){
        try{ window.addMemoryNotice(mem); }catch{}
        try{ if(typeof window.updateContextMeter==='function') window.updateContextMeter(); }catch{}
        return;
      }
    }
  }catch{}

...
const modelsBtn=byId('modelsBtn'),modelsPop=byId('modelsPop'),modelTitle=byId('modelTitle'),home=byId('home'),chat=byId('chat'),thread=byId('thread'),threadInner=byId('threadInner'),homeInput=byId('homeInput'),homeSend=byId('homeSend'),chatInput=byId('chatInput'),chatSend=byId('chatSend'),newChatBtn=byId('newChatBtn'),chatsList=byId('chatsList'),profileBox=byId('profileBox'),userNameEl=byId('userName'),avatarInitial=byId('avatarInitial'),nameEditWrap=byId('nameEditWrap'),nameInput=byId('nameInput'),homeMicBtn=byId('homeMicBtn'),chatMicBtn=byId('chatMicBtn'),settingsOverlay=byId('settingsOverlay'),settingsCloseBtn=byId('settingsCloseBtn'),searchBtn=byId('searchChats');
...
function initTheme(){const savedTheme=localStorage.getItem('theme');setTheme(savedTheme||'light')}
...
function addUserMessage(text){
  ...
  row.innerHTML=`<div class=\"center\" style=\"display:flex;gap:12px;justify-content:flex-end;align-items:flex-start\"><div class=\"bubble\">${escapeHTML(text)}</div><div class=\"avatar\">ðŸ§‘</div></div>`;
...
function addAssistantMessage(markdownText,finalize=!1){
  ...
  row.querySelector('[data-action="regen"]').addEventListener('click', () => {
    try {
      let currentNode = row;
      let prevUserMsgEl = null;
      while (currentNode.previousElementSibling) {
        currentNode = currentNode.previousElementSibling;
        if (currentNode.classList.contains('user')) {
          prevUserMsgEl = currentNode;
          break;
        }
      }
      ...
      if (prevUserMsgEl) {
        ...
        // Remove all messages after the found user message
        let toRemove = [];
        let sibling = prevUserMsgEl.nextElementSibling;
        while(sibling) {
...
// The "Flashcards" feature has been removed.
...
// This function is now handled by app.js to provide full-text search
// function renderSearchResults(q){ ... }

// The "Memories" feature has been removed.
// This is now handled in app.js
...
// --- Memories Modal ---
// The "Memories" modal and related functionality have been removed.

// --- Settings Modal ---
function openSettingsModal(){
  if(!currentSettings) return;
  settingsOverlay.classList.add('visible');
  const setThemeSel=byId('setTheme'), setAnimChk=byId('setAnimation'), setDisplayName=byId('setDisplayName'), setUserNameAI=byId('setUserNameAI'), setDefaultModel=byId('setDefaultModel'), setLanguageSel=byId('setLanguage'), setContextMeter=byId('setContextMeter'), setDisableSysPrompt=byId('setDisableSysPrompt'), setLlamaCppServerPath=byId('setLlamaCppServerPath'), setLlamaCppModelId=byId('setLlamaCppModelId');
  ...
  try{ if(setLlamaCppServerPath) setLlamaCppServerPath.value = currentSettings.llamacpp_server_path || ''; }catch{}
  try{ if(setLlamaCppModelId) setLlamaCppModelId.value = currentSettings.llamacpp_model_id || ''; }catch{}

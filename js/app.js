...
async function handleSend(inputEl,isHome=!1){
  ...
  if(!text)return;
  // Clear input immediately so user can type a new prompt while streaming
  inputEl.value='';
  updateContextMeter();
...
function buildSystemPreview(settings,memories){
  ...
  try{if(settings?.personality?.enabled){const picks=(settings.personality.selected||[]).slice(0,3);if(picks.length){parts.push("Personality blend:\n"+picks.map(p=>`- ${p}: ${PERSONALITY_LIBRARY[p]||''}`).join("\n")+"\nCombine the above into one coherent style without conflict.");}}}catch{}
  try{const v=settings?.verbosity||'High';const map={'Very High':'extremely detailed and expansive; long, engaging, but always on-topic','High':'highly verbose; long and engaging while staying strictly on-topic','Medium':'balanced detail; clear and complete without excess','Low':'concise; short, direct answers','Very Low':'minimal; terse bullets or one-liners when possible',};if(map[v])parts.push(`Verbosity target: ${v} — ${map[v]}.`);}catch{}
  parts.push("Personalization: Read the user's vibe and adapt tone — be hyped/crazy when they are, serious when they are, chill when they are, and fun when they are. Always align with selected personalities and keep it engaging without fluff.");
  ...
  if(Array.isArray(memories)&&memories.length){parts.push("User memories:\n"+memories.map(m=>`- ${m}`).join("\n"));}
  return parts.join("\n\n");
}
